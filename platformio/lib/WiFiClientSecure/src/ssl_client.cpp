...
#define ROE(x,msg) { if (((x)<0)) { log_e("LWIP Socket config of " msg " failed."); return -1; }}
    ...
    // esp32-weather-epd: prefer X25519 and P-256 for the ECDHE key exchange.
    // mbedTLS's default list offers secp521r1 first, and servers that honor
    // the client's preference (api.open-meteo.com does) then pick P-521,
    // which the ESP32 computes in software at ~1.5 s per handshake. Servers
    // with their own preference (weather.gov, Google) already choose X25519.
    // Measured on the E1002: Open-Meteo handshakes 2.7-3.0 s before this.
    {
        ...
        mbedtls_ssl_conf_curves(&ssl_client->ssl_conf, preferred_curves);
    }
